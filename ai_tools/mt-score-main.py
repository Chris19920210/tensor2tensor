...
if __name__ == "__main__":
    ...
    print("%s/chi_char_segment.pl -type xml < %s > %s/hyp.seg.sgm"%(bin, args.hyp_sgm, work_dir))
    print("%s/chi_char_segment.pl -type xml < %s > %s/ref.seg.sgm"%(bin, args.ref_sgm, work_dir))
    print("%s/mteval-v11b.pl -s %s -r %s/ref.seg.sgm -t %s/hyp.seg.sgm -c > %s/bleu " % (bin, args.src_sgm, work_dir, work_dir,  work_dir))
