...
def beam_search(symbols_to_logits_fn,
  ...
  def grow_topk(i, alive_seq, alive_log_probs, states):
    ...
    flat_logits_list = symbols_to_logits_fn(flat_ids[:, -1:], states, tf.to_float(i + 1))

    logits_list = nest.map_structure(
      lambda flat_logits: tf.reshape(flat_logits[0], (batch_size, beam_size, -1)),
      flat_logits_list)

    states_list = nest.map_structure(
      lambda flat_logits: tf.reshape(tf.transpose(flat_logits[1], [1, 0, 2, 3]), (batch_size, beam_size, -1)),
      flat_logits_list)

    candidate_log_probs = nest.map_structure(lambda logits: common_layers.log_prob_from_logits(logits),
                                             logits_list)
    ...
    if not use_tpu:
      ...
      if states:
        states = nest.map_structure(
            lambda state: tf.gather_nd(state, topk_coordinates), states_list)
